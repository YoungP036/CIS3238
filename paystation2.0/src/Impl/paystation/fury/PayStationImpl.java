...
public class PayStationImpl implements PayStation{
    ...
    private final RateStrategy rs;// = new LinearRateStrategy();
     ...
     * This is the only place that can increase timeBought, so sayeth i
     */
	...
	if(coinMap.containsKey(coinValue))
	   coinMap.replace(coinValue, coinMap.get(coinValue)+1);
	else	    
	    coinMap.put(coinValue,1);
	
	insertedSoFar+=coinValue;
	timeBought=rs.rateCalculation(insertedSoFar);
